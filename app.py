...
# ---------------- UI ----------------
st.set_page_config(page_title="Multimodal Disaster AI", page_icon="🌊", layout="centered")
st.title("🌍🔥 Multimodal Disaster Response — Demo")
st.caption(f"Device: **{DEVICE}** • DistilBERT + ResNet18 • Late-fusion α,thr from artifacts/fusion/alpha.json")
...
with tab1:
    ...
    with col1:
        text_input = st.text_area("Text (optional)", height=140, placeholder="Enter a tweet or message...")
    with col2:
        img_file = st.file_uploader("Image (optional)", type=["jpg","jpeg","png","webp","bmp"])

    alpha = st.slider("Fusion weight α (text vs image)", 0.0, 1.0, value=float(alpha_default), step=0.05)
    thr = st.slider("Decision threshold", 0.05, 0.95, value=float(thr_default), step=0.01)
    ...
    if st.button("Predict", type="primary"):
        ...
        if (p_text is None) and (p_image is None):
            ...
            if p_text is None: p_text = 0.0
            if p_image is None: p_image = 0.0
            ...
            with colC:
                st.metric("Fused p(disaster)", f"{p_fused:.3f}", help="α·text + (1-α)·image")
...
with tab2:
    ...
    st.write("Upload a CSV with columns: **text** (optional) and **image** (optional path/filename). Optionally upload a **ZIP** of images whose filenames match the `image` column.")

    csv_file = st.file_uploader("CSV", type=["csv"], key="csv")
    zip_file = st.file_uploader("Images ZIP (optional)", type=["zip"], key="zip")

    alpha_b = st.slider("Fusion weight α (text vs image) [batch]", 0.0, 1.0, value=float(alpha_default), step=0.05, key="alpha_b")
    thr_b = st.slider("Decision threshold [batch]", 0.05, 0.95, value=float(thr_default), step=0.01, key="thr_b")
    ...
    if st.button("Run Batch", type="primary", key="runbatch") and csv_file is not None:
        ...
        if ("text" not in df.columns) and ("image" not in df.columns):
            ...
            for _, row in df.iterrows():
                t = str(row["text"]) if "text" in df.columns and not pd.isna(row["text"]) else None
                img_path = None
                ...
                probs_text.append(pt); probs_image.append(pi); probs_fused.append(pf)
