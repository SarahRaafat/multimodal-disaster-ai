...
def main():
    ap = argparse.ArgumentParser(
        description="Prepare image dataset: dedupe, resize, split into train/val/test."
    )
    ap.add_argument(
        "--input-dir",
        type=str,
        required=True,
        help="Directory containing class subfolders (e.g., flooded/, non_flooded/).",
    )
    ap.add_argument(
        "--output-dir", type=str, required=True, help="Output root (e.g., image_data)."
    )
    ap.add_argument(
        "--img-size", type=int, default=224, help="Final square image size."
    )
    ap.add_argument(
        "--max-side",
        type=int,
        default=512,
        help="Limit longest side before square crop/resize.",
    )
    ap.add_argument(
        "--max-per-class",
        type=int,
        default=500,
        help="Cap per class to keep things lightweight.",
    )
    ...
    ap.add_argument(
        "--dedupe", action="store_true", help="Enable file hash deduplication."
    )
    ...
    if not items:
        raise SystemExit(
            f"No images found under {in_root}. Expecting class subfolders like 'flooded/' and 'non_flooded/'."
        )
    labels = sorted({label for _, label in items})
    by_label = {label_key: [] for label_key in labels}
    for p, label_key in items:
        by_label[label_key].append(p)

    before_counts = {label_key: len(v) for label_key, v in by_label.items()}

    if args.dedupe:
        seen_hashes = set()
        new_by_label = {label_key: [] for label_key in labels}
        for label_key, paths in by_label.items():
            for p in paths:
                h = file_hash(p)
                if h in seen_hashes:
                    # skip duplicate
                    continue
                seen_hashes.add(h)
                new_by_label[label_key].append(p)
        by_label = new_by_label  # <- IMPORTANT: actually use the deduped lists
        dedupe_enabled = True
    else:
        dedupe_enabled = False

    after_counts = {label_key: len(v) for label_key, v in by_label.items()}

    # number of files removed by dedupe (0 if dedupe disabled)
    duplicates = sum(
        before_counts[label_key] - after_counts[label_key] for label_key in labels
    )
    ...
    for split_name, label_map in splits.items():
        for label, paths in label_map.items():
            for src in paths:
                ...
                dst = out_root / rel
                resize_and_copy(
                    src, dst, max_side=args.max_side, img_size=args.img_size
                )
