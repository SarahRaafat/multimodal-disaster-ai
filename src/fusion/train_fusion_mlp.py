...
def main():
    ...
    ap.add_argument(
        ...
        help="Embeddings root folder",
    )
    ap.add_argument(
        "--mode",
        type=str,
        default="concat_naive",
    ...
    ap.add_argument(
        "--class-weights",
        action="store_true",
        help="Use class weights from train split",
    )
    args = ap.parse_args()
    ...
    # Build splits
    txt_train = emb_dir / "text_train.npz"
    img_train = emb_dir / "image_train.npz"
    txt_val = emb_dir / "text_val.npz"
    img_val = emb_dir / "image_val.npz"
    txt_test = emb_dir / "text_test.npz"
    img_test = emb_dir / "image_test.npz"
    ...
    if args.class_weights:
        ...
        s = w0 + w1
        w = torch.tensor([w0 / s, w1 / s], dtype=torch.float32, device=device)
        print(
            f"[loss] Using class weights = [{w[0].item():.6f}, {w[1].item():.6f}]  (from counts={dict(c)})"
        )
