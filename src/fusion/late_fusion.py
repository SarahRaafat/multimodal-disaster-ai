...
def metrics_from_probs(y, p, thr=0.5):
    ...
    return {"accuracy":acc, "precision":prec, "recall":rec, "f1":f1, "confusion_matrix":cm, "threshold":thr}
...
def fit_platt(p_val, y_val):
    ...
    eps = 1e-6
    logits = np.log(np.clip(p_val, eps, 1-eps) / np.clip(1-p_val, eps, 1-eps)).reshape(-1,1)
...
def main():
    ...
    # --- VAL probs
    p_txt_val, y_txt_val = text_probs(Path(args.text_data_dir)/"val.csv", args.distil_dir, args.batch_size, args.max_len, device)
    p_img_val, y_img_val = image_probs(Path(args.image_data_dir)/"val", args.resnet_ckpt, args.batch_size, args.img_size, device)
    ...
    # --- TEST probs
    p_txt_test, y_txt_test = text_probs(Path(args.text_data_dir)/"test.csv", args.distil_dir, args.batch_size, args.max_len, device)
    p_img_test, y_img_test = image_probs(Path(args.image_data_dir)/"test", args.resnet_ckpt, args.batch_size, args.img_size, device)
    ...
    print(f"[val] best fusion: alpha={best['alpha']:.2f}, thr={best['thr']:.2f}, f1={best['f1']:.4f}")
    ...
    cal = {
        "text":  {"ece": ece_score(y_test, p_txt_test_cal), "brier": brier_score(y_test, p_txt_test_cal)},
        "image": {"ece": ece_score(y_test, p_img_test_cal), "brier": brier_score(y_test, p_img_test_cal)},
        "fused": {"ece": ece_score(y_test, p_f_test),       "brier": brier_score(y_test, p_f_test)},
    }

    # save artifacts
    with open(out/"alpha.json","w") as f: json.dump({"alpha":best["alpha"], "threshold":best["thr"]}, f, indent=2)
    with open(out/"late_val_best.json","w") as f: json.dump({"alpha":best["alpha"], "thr":best["thr"], "val_f1":best["f1"]}, f, indent=2)
    with open(out/"late_test_metrics.json","w") as f: json.dump(test_metrics, f, indent=2)
    with open(out/"calibration_test.json","w") as f: json.dump(cal, f, indent=2)
