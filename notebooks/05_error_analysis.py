...
def summarize_errors(y, p, thr=0.5):
    ...
    return {
        ...
        "fn": int(fn),
        "tn": int(tn),
        "precision_recall_f1": tuple(
            precision_recall_fscore_support(y, pred, average="binary", zero_division=0)[
                :3
            ]
        ),
    }
...
def main():
    ...
    # ---------- TEXT ----------
    df_test, p_text_test, y_text_test = text_probs(
    ...
    df_val, p_text_val, y_text_val = text_probs(
    ...
    # ---------- IMAGE ----------
    p_img_test, y_img_test, img_paths_test, cls_names = image_probs(
    ...
    p_img_val, y_img_val, img_paths_val, _ = image_probs(
    ...
    # save grids
    save_image_error_grid(
        img_paths_test,
        y_img_test,
        p_img_test,
        best_img["thr"],
        cls_names,
        out / "image_fp_grid.png",
        "IMAGE – False Positives (pred=disaster, true=non)",
    )
    save_image_error_grid(
        img_paths_test,
        y_img_test,
        ...
        best_img["thr"],
        cls_names,
        out / "image_fn_grid.png",
        "IMAGE – False Negatives (pred=non, true=disaster)",
    )
    ...
    notes = {
        ...
        "image_summary": m_img,
        "fusion_alpha_thr": (
            alpha_thr if Path(args.fusion_dir, "alpha.json").exists() else None
        ),
    }
