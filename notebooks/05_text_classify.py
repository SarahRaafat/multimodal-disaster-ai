...
try:
    thr = json.loads((Path(args.model_dir)/"best_threshold.json").read_text()).get("threshold", 0.5)
except Exception:
    thr = 0.5
...
with torch.no_grad():
    for i in range(0, len(df), 32):
        enc = tok(df["text"].astype(str).iloc[i:i+32].tolist(),
                  truncation=True, padding=True, max_length=args.max_len, return_tensors="pt")
        logits = model(**enc).logits
        probs.extend(softmax1(logits))

df_out = df.copy()
df_out["p_disaster"] = probs
df_out["pred"] = (df_out["p_disaster"] >= thr).map({True:"disaster", False:"non_disaster"})
