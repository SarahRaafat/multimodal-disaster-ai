...
def text_saliency_examples(
    csv_path, model_dir, out_html, text_col="text", max_len=160, device="cpu", k=6
):
    tok = DistilBertTokenizerFast.from_pretrained(model_dir)
    model = (
        DistilBertForSequenceClassification.from_pretrained(model_dir).to(device).eval()
    )
    ...
    with torch.no_grad():
        for i in range(0, len(df), 32):
            ...
            enc = tok(
                chunk[text_col].astype(str).tolist(),
                truncation=True,
                padding=True,
                max_length=max_len,
                return_tensors="pt",
    ...
    for idx in chosen:
        ...
        html_parts.append(
            f"<div class='ex'><div class='p'><b>Pred:</b> {label} &nbsp; <b>p(disaster)</b>= {p:.3f}</div>"
        )
        html_parts.append(
            "<div style='line-height:1.8'>" + " ".join(vis) + "</div></div>"
        )
...
class GradCAM:
    def __init__(self, model, target_layer):
        ...
        self.h2 = target_layer.register_backward_hook(
            self._backward_hook
        )  # for torchvision<=0.15
...
def gradcam_on_images(image_root, ckpt_path, out_dir, img_size=224, device="cpu", n=6):
    ...
    def overlay_heatmap(img_arr, cam):
        cam_up = cam
        # resize cam to image
        cam_up = Image.fromarray((cam_up * 255).astype(np.uint8)).resize(
    ...
    for idx in chosen:
        ...
        ax[1].imshow(overlay)
        ax[1].axis("off")
        ax[1].set_title("Grad-CAM (disaster)")
        fig.tight_layout()
...
def main():
    ...
    ap.add_argument(
        "--image-ckpt", type=str, default="artifacts/resnet18_ft/resnet18.pt"
    )
    ...
    try:
        text_saliency_examples(
            args.text_csv,
            args.text_model_dir,
            text_html,
            text_col="text",
            max_len=args.text_maxlen,
            device=device,
            k=args.text_k,
        )
