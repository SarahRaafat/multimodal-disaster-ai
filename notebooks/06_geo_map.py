...
def main():
    ...
    ap.add_argument(
        "--model-dir", type=str, default="artifacts/distilbert"
    )  # or artifacts/distilbert_ft
    ...
    for c in [args.text_col, args.lat_col, args.lon_col]:
        if c not in df.columns:
            raise SystemExit(
                f"[error] '{c}' column not found in {csv_path}. "
    ...
    df["pred"] = (df["p_disaster"] >= thr).map(
        {True: "disaster", False: "non_disaster"}
    )

    # Center map at mean lat/lon
    lat0 = float(df[args.lat_col].mean())
    lon0 = float(df[args.lon_col].mean())
    ...
    for _, row in df.iterrows():
        ...
        folium.CircleMarker(
            ...
            color=color_row(p),
            fill=True,
            fill_color=color_row(p),
            fill_opacity=0.8,
            tooltip=f"{pred} • p={p:.2f}",
            popup=folium.Popup(
                html=f"<b>{pred}</b> (p={p:.2f})<br/><hr/>{txt}", max_width=360
            ),
        ).add_to(mc)
